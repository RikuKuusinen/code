...
const App = () => {
  ...
  return (
    <div>
      <Anecdotes anecdotes={anecdotes} selected={selected} points={points[selected]}></Anecdotes>
      <Anecdotes text="Most voted" anecdotes={anecdotes} selected={mostVoted} points={points[mostVoted]}></Anecdotes>

      <Button handleClick={() => setSelected(takeRandomNumber())} text="Take random attribute"/>
      <Button handleClick={() => {setPoints(setPointsToSelected(points,selected), setMostVoted(setMostVotedPoints(points, mostVoted)))}} text="Vote" />
    </div>
  )
}
...
const Anecdotes = ({anecdotes, selected, points, text}) => {
  console.log("valittu",selected);
      ...
      return (<div><br/>{text}<div>{anecdotes[selected]}</div>
       <div>Yhteispisteet: {points}</div></div>
       )
} 
